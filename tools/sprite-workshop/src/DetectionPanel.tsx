...
type Props = {
  ...
  suggestions: Region[]; selected: Region | null; onSelected: (id: string) => void
  onGenerate: () => void; onEdit: (key: 'x' | 'y' | 'width' | 'height', value: string) => void
  onAccept: (id: string) => void; onReject: (id: string) => void; onClear: () => void
}

export function DetectionPanel(props: Props) {
  const { enabled, mode, onMode, grid, onGrid, joinGap, onJoinGap, minPixels, onMinPixels, suggestions, selected, onSelected, onGenerate, onEdit, onAccept, onReject, onClear } = props
