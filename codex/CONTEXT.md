...
The Rust/Tauri backend is the trusted boundary for project and filesystem operations. Model-generated tool arguments are untrusted. Repository context is retrieved progressively through controlled read-only tools rather than sending the entire project automatically; inspection is bounded and protects paths and sensitive files. Elma may propose bounded exact replacements to existing text files, but only the user's Apply action may write them. Rust validates the proposal, retains the trusted original snapshot, and rejects stale application. Commands and Git mutations remain unavailable. Preserve local-first operation, safe defaults, and user reviewability when adding capabilities. Refer to [REPO-MAP.md](REPO-MAP.md) for current file ownership and to `src-tauri/src/repository.rs` for implementation-specific limits.

Elma's default communication/personality instructions are kept separate from core agent, protocol, and safety instructions. Future settings may customize communication, but must never override or weaken the trusted agent boundary.

Agent Debug Mode is an observability-only, in-memory trace of the latest request. When enabled, the same trace is available in the UI and development terminal; it must not alter prompts, schemas, model settings, permissions, or agent behavior.
