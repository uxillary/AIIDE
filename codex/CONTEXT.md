...
M08A exists only on `image-generation`. It implements a separate ComfyUI/SDXL image-provider path, one active or reviewable job, real queue/generation states, PNG preview, Save/Reject approval, protected project-relative saving, collision naming, temporary cleanup, prompt-specific cancellation where supported, and a minimal Chat/Image composer toggle. It now also contains disabled managed-runtime foundations: explicit external/managed configuration migration, the pinned C1.6 manifest and integrity contracts, consent/transfer/resume validation, transactional app-owned storage, bounded in-process 7z extraction, and a Windows suspended-process/Job Object supervisor. Managed acquisition and execution remain disabled and unexposed pending trust, legal, release, and live-acceptance approval. ComfyUI has not completed the real GPU acceptance flow, so output quality and memory behaviour remain unverified. Its design file is `codex/M08A-IMAGE-GENERATION-DESIGN.md` on that branch.
