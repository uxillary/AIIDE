...
export function ChangesPanel({ projectPath, hasRepository, proposal, history, busy, error, primaryElma, onApply, onReject, onGitStatus }: { projectPath: string | null; hasRepository: boolean; proposal: PendingProposal | null; history: ChangeHistoryEntry[]; busy: boolean; error: string | null; primaryElma: Parameters<typeof ElmaPresence>[0] | null; onApply: () => void; onReject: () => void; onGitStatus: (status: GitStatus) => void }) {
  return <aside className="changes-panel" aria-label="Changes"><div className="main-label">CHANGES</div>{primaryElma && <ElmaPresence {...primaryElma} className="changes-elma-status" />}<section className="proposal-panel" aria-label="AiiDE proposal changes"><h2 className="panel-section-label">AIIDE PROPOSALS</h2>{proposal ? <div className="changes-review">
  ...
  {!proposal && history.length === 0 && !error && <div className="changes-empty"><span className="changes-empty-icon" aria-hidden="true">✦</span><h2>No proposal changes</h2><p>Edits Elma prepares appear here. Git changes are tracked separately below.</p></div>}</section>
