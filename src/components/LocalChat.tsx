...
export function LocalChat({ projectOpen, projectBusy, onOpenProject, onProposal, changeState }: { projectOpen: boolean; projectBusy: boolean; onOpenProject: () => void; onProposal: (proposal: PendingProposal) => void; changeState: 'working' | 'success' | 'error' | null }) {
  ...
  return <main className="main-panel">
    ...
    {projectOpen && <div className="chat-notice">Elma can inspect your project and prepare changes. Files are modified only after you approve.</div>}
    ...
    <div className="chat-history">
      ...
      {projectOpen && ready && messages.length === 0 && <div className="chat-empty project-start"><Elma state="idle" size="hero" /><span className="eyebrow">READY TO HAVE A LOOK</span><h2>What are we working on?</h2><p>I can inspect this project, explain what I find, or prepare a focused edit for review.</p><div className="starter-prompts">{STARTER_PROMPTS.map(starter => <button key={starter} onClick={() => setPrompt(starter)}>{starter}<span aria-hidden="true">→</span></button>)}</div></div>}
      {messages.map((message, index) => <div className={`chat-message chat-message-${message.role}`} key={index}><div className="chat-speaker"><span>{message.role === 'user' ? 'YOU' : 'ELMA'}</span>{message.role === 'assistant' && message.model && <small>{message.model}</small>}</div>{Boolean(message.activity?.length) && <details className="message-activity"><summary>Inspected {message.activity?.length} items</summary><ul>{message.activity?.map((item, step) => <li key={step}>✓ {item.label}</li>)}</ul></details>}<div className="message-body">{message.content}</div></div>)}
    ...
    <div className="chat-composer">{error && <div role="alert" className="composer-error">{error}</div>}<div className="composer-row"><textarea aria-label="Prompt" className="prompt-input" value={prompt} disabled={!ready || loading} maxLength={12000} placeholder={ready ? 'Ask Elma about this project…' : 'Connect Ollama and select a model to chat'} onChange={event => setPrompt(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send() } }} /><button className="primary-button send-button" onClick={() => void send()} disabled={!ready || !prompt.trim() || loading}>Send</button></div><p className="composer-hint"><kbd>Enter</kbd> send <span>·</span> <kbd>Shift</kbd> + <kbd>Enter</kbd> new line <span>·</span> Session-only chat</p></div>
  </main>
}
