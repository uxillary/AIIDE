...
export interface ImageProvider {
  ...
  configure(endpoint: string, modelId: string, checkpoint?: string): Promise<ImageEngineStatus>
  ...
  regenerate(jobId: string): Promise<ImageJob>
  reject(jobId: string): Promise<void>
  save(jobId: string, relativePath: string): Promise<string>
  saveAs(jobId: string, absolutePath: string): Promise<string>
